  ...
  stages {
    ...
    stage('Push to ECR') {
      steps {
            ...
            sh "docker tag ${ECR_REPO}:latest ${ECR_URL}/${ECR_REPO}:${GIT_COMMIT:0:6}"
            sh "docker push ${ECR_URL}/${ECR_REPO}:latest"
            sh "docker push ${ECR_URL}/${ECR_REPO}:${GIT_COMMIT:0:6}"
            }   
      }
     stage("Deploy") {
          steps {
              ...
              sh 'ecs deploy -t ${GIT_COMMIT:0:6} --no-deregister --region us-east-1 --timeout 300 --task ${SERVICE} ${CLUSTER_NAME} ${SERVICE}'

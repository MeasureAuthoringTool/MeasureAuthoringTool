    ...
    stage('Push to ECR') {
      steps {
            sh '''
               ...
               docker build -t ${ECR_REPO} --build-arg STOREPASS="${STOREPASS}" --build-arg NR_APM_NAME="${NR_APM_NAME}" --build-arg NR_APM_KEY="${NR_APM_KEY}" --build-arg WAR_PATH="sbx#MeasureAuthoringTool.war" .

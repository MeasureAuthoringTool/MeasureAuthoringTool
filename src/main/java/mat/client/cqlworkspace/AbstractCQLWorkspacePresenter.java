...
public abstract class AbstractCQLWorkspacePresenter {
    ...
    protected CQLCode buildCQLCodeFromCodesView(String codeName) {
        logger.log(Level.INFO,"Entering buildCQLCodeFromCodesView");
        ...
        if (MatContext.get().isCurrentModelTypeFhir()) {
            logger.log(Level.INFO,"isCurrentModelTypeFhir");
            VSACCodeSystemDTO vsacDto = MatContext.get().getOidToVSACCodeSystemMap().get("urn:oid:" + codesView.getCodeSystemOid());
            logger.log(Level.INFO,"vsacDto: " + vsacDto);
            if (vsacDto != null) {
                refCode.setCodeSystemOID(vsacDto.getUrl());
                if (isCodeSystemVersionIncluded) {
                    logger.log(Level.INFO,"isCodeSystemVersionIncluded");

                    if ("SNOMEDCT".equals(refCode.getCodeSystemName())) {
                        refCode.setCodeSystemVersionUri("http://snomed.info/sct/731000124108/version/" +
                                removeDashes(refCode.getCodeSystemVersion()));
                    } else {
                        refCode.setCodeSystemVersionUri(refCode.getCodeSystemVersion());
                    }
                }
            } else {
                messagePanel.getErrorMessageAlert().createAlert("Could not find VSAC to Fhir 4.0.1 mapping for oid " +
						codesView.getCodeSystemOid() + ". Please contact the help desk.");
            }
        } else {
    ...
    protected String removeDashes(String s) {
        ...
        if (nextDash >= 0) {
            String result = s.substring(0, nextDash) + s.substring(nextDash + 1);
            return removeDashes(result);
        } else {
            return s;
        }
    }
    ...
    protected void copyCodes() {
        ...
        if (cqlWorkspaceView.getCodesView().getCodesSelectedList() != null && cqlWorkspaceView.getCodesView().getCodesSelectedList().size() > 0) {
            ...
            logger.log(Level.INFO,"Copying codes: " + cqlWorkspaceView.getCodesView().getCodesSelectedList());

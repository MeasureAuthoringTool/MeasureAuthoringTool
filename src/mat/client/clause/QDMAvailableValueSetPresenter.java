...
public class QDMAvailableValueSetPresenter implements MatPresenter {
	...
	private MatValueSet currentMatValueSet;// To Do Sowmaya to Set this value
											// set.
	...
	public QDMAvailableValueSetPresenter(
		...
		this.appliedQDMList = (ArrayList<QualityDataSetDTO>) qdsAppliedListPresenterDisplay
				.getAllAppliedQDMList();
		...
		MatContext
				...
				.addHandler(OnChangeOptionsEvent.TYPE,
						new OnChangeOptionsEvent.Handler() {
							@Override
							public void onChangeOptions(
									OnChangeOptionsEvent event) {
								final CodeListSearchDTO codeList = currentCodeListResults
										.getSelectedCodeList();
								searchDisplay.scrollToBottom();
								searchDisplay.getApplyToMeasureSuccessMsg()
										.setMessage("");
								searchDisplay.getSpecificOccurrenceInput()
										.setValue(false);// Unchecking the
															// specific
															// occurrence
															// checkbox on
															// change of radio
															// options.
								if (codeList != null) {
									String codeListCategory = codeList
											.getCategoryDisplay();
									if (codeListCategory
											.equalsIgnoreCase(ConstantMessages.ATTRIBUTE)
											|| codeListCategory
													.equalsIgnoreCase(ConstantMessages.MEASUREMENT_TIMING)) {
										searchDisplay.getDataTypeInput()
												.setEnabled(false);
										searchDisplay
												.getSpecificOccurrenceInput()
												.setEnabled(false);
										searchDisplay.getApplyToMeasure()
												.setEnabled(true);// enable only
																	// the
																	// ApplyToMeasure
																	// Button in
																	// these
										...
										populateQDSDataType(codeList
												.getCategoryCode());
										searchDisplay.getDataTypeInput()
												.setEnabled(true);// Enable both
																	// the
																	// datatype
																	// dropdown
																	// and
		...
		searchDisplay.getSearchButton().addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				int filter = searchDisplay.getValueSetSearchFilterPanel()
						.getSelectedIndex();
								...
								.getMeasureXMLForAppliedQDM(
										...
										new AsyncCallback<ArrayList<QualityDataSetDTO>>() {
											...
											public void onSuccess(
												...
												currentMatValueSet = searchDisplay
		...
		searchDisplay.getUserDefinedCancel().addClickHandler(
				new ClickHandler() {
	...
	/***
	 * Method to find if selected Available value set is a valid modifiable
	 * selection. If yes, then call to updateAppliedQDMList method is made.
	 * 
	 * */
	...
	private void modifyValueSetQDM() {
		...
		// CodeListSearchDTO modifyWithDTO =
		// currentCodeListResults.getLastSelectedCodeList();
		...
		if (modifyValueSetDTO != null && modifyWithDTO != null) {
			...
			if (modifyValueSetDTO.getDataType().equalsIgnoreCase(
					ConstantMessages.ATTRIBUTE)
					|| dataTypeText
							.equalsIgnoreCase(ConstantMessages.ATTRIBUTE)) {
				if (dataTypeText.equalsIgnoreCase(modifyValueSetDTO
						.getDataType())) {
					updateAppliedQDMList(modifyWithDTO, null,
							modifyValueSetDTO, dataType, isSpecificOccurrence,
							false);
				} else {
					if (ConstantMessages.ATTRIBUTE
							.equalsIgnoreCase(dataTypeText)) {
						searchDisplay
								.getErrorMessageDisplay()
								.setMessage(
			...
			searchDisplay.getErrorMessageDisplay().setMessage(
					"Please select atleast one applied QDM to modify.");
			setEnabled(true);
		}
	}
	...
	private void modifyQDMWithOutValueSet() {
		...
		if ((searchDisplay.getUserDefinedInput().getText().trim().length() > 0)
			...
			modifyWithDTO
					.setName(searchDisplay.getUserDefinedInput().getText());
			String dataType = searchDisplay.getDataTypeValue(searchDisplay
					.getAllDataTypeInput());
			String dataTypeText = searchDisplay.getDataTypeText(searchDisplay
					.getAllDataTypeInput());
			if (modifyValueSetDTO.getDataType().equalsIgnoreCase(
					ConstantMessages.ATTRIBUTE)
					|| dataTypeText
				...
				if (dataTypeText.equalsIgnoreCase(modifyValueSetDTO
						.getDataType())) {
					updateAppliedQDMList(null, modifyWithDTO,
							modifyValueSetDTO, dataType, false, true);
				} else {
					if (ConstantMessages.ATTRIBUTE
							.equalsIgnoreCase(dataTypeText)) {
						searchDisplay
				...
				updateAppliedQDMList(null, modifyWithDTO, modifyValueSetDTO,
						dataType, false, true);
			}
		} else {
			searchDisplay
	 ...
	 * check's for attributes and non attributes , Occurrence and non occurences
	 * are done in this method. This method returns modified and ordered list of
	 * all applied QDM elements.This method also makes call to updateMeasureXML
	 * method.
	 * 
	 * **/

	private void updateAppliedQDMList(final MatValueSet matValueSet,
			final CodeListSearchDTO codeListSearchDTO,
			final QualityDataSetDTO qualityDataSetDTO, String dataType,
			Boolean isSpecificOccurrence, final boolean isUSerDefined) {
		MatContext
				.get()
				.getCodeListService()
				.updateCodeListToMeasure(
						MatContext.get().getCurrentMeasureId(), dataType,
						matValueSet, codeListSearchDTO, qualityDataSetDTO,
						isSpecificOccurrence, appliedQDMList, isUSerDefined,
						new AsyncCallback<SaveUpdateCodeListResult>() {
							...
							public void onSuccess(
								...
								if (result.getFailureReason() == 7) {
									...
									updateMeasureXML(result.getDataSetDTO(),
											qualityDataSetDTO, isUSerDefined);
								}
							}
						});
	...
	private void updateMeasureXML(QualityDataSetDTO qualityDataSetDTO2,
			QualityDataSetDTO qualityDataSetDTO, final boolean isUserDefined) {
		MatContext
				.get()
				.getMeasureService()
				.updateMeasureXML(qualityDataSetDTO2, qualityDataSetDTO,
						MatContext.get().getCurrentMeasureId(),
	...
	private void search(String searchText, int startIndex, final int pageSize,
		...
		MatContext
				.get()
				.getCodeListService()
				.search(lastSearchText, startIndex, Integer.MAX_VALUE,
						sortColumn, isAsc, defaultCodeList, filter,
						new AsyncCallback<ManageCodeListSearchModel>() {
							@Override
							public void onSuccess(
									ManageCodeListSearchModel result) {
								...
								QDSSearchResult.setResultsTotal(result
										.getResultsTotal());
								...
								sru = null;
								searchDisplay.buildQDSDataTable(
										QDSSearchResult, true);
							...
							public void onFailure(Throwable caught) {
								searchDisplay
										.getErrorMessageDisplay()
										.setMessage(
												"Problem while performing a search");
								showSearchingBusy(false);
							}
						});
	}
	...
	private void populateQDSDataType(String category) {
		MatContext
				.get()
				.getListBoxCodeProvider()
				.getQDSDataTypeForCategory(category,
						new AsyncCallback<List<? extends HasListBox>>() {
	...
	public void setEnabled(boolean enabled) {
		...
		// "Specific Occurrence" check box, and the "Apply to Measure" button
		// are to be enabled
		boolean applyToMeasure = false;
		boolean specificOccurrence = false;
		boolean dataTypeInput = false;
		final CodeListSearchDTO codeList = currentCodeListResults == null ? null
				: currentCodeListResults.getSelectedCodeList();
		if (enabled && codeList != null) {
			boolean dataTypeSelected = searchDisplay.getDataTypeInput()
					.getSelectedIndex() > 0;
			if (!codeList.getName().equalsIgnoreCase(
					ConstantMessages.MEASUREMENT_PERIOD)
					&& !codeList.getName().equalsIgnoreCase(
							ConstantMessages.MEASUREMENT_START_DATE)
					&& !codeList.getName().equalsIgnoreCase(
							ConstantMessages.MEASUREMENT_END_DATE)
					&& !codeList.getCategoryDisplay().equalsIgnoreCase(
							ConstantMessages.ATTRIBUTE)) {
		...
		// apply to measure
		((Button) searchDisplay.getAddToMeasureButton())
				.setEnabled(applyToMeasure);
		// specific occurrence
		searchDisplay.getSpecificOccurrenceInput().setEnabled(
				specificOccurrence);
		// select data type

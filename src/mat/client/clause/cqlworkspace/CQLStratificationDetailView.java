...
public class CQLStratificationDetailView {
	...
	private Grid generateStratumGrid(StratificationsObject stratificationsObject) {
		...
		for (int i = 0; i < stratumClauses.size(); i++) {
			...
			deleteButton.getElement().setAttribute("aria-label", "click delete button to delete "+ populationClauseObject.getDisplayName());
	...
	private Grid generateStratificationGrid(StratificationsObject stratificationsObject) {
		...
		addNewStratum.getElement().setAttribute("aria-label", "Click this button to add new Stratum under " + stratificationsObject.getDisplayName());

...
public class SimpleEMeasureServiceImpl implements SimpleEMeasureService {
	...
	// CONVERSION_FILE_1="xsl/db2hqmf_08262010_1.xsl";
	...
	// private static final String
	// SUPPLEMENTDATAELEMENT="supplementalDataElements";
	...
	private static final String XPATH_ALL_MSR_OBS_ELEMENTREF_ID = "/measure/measureObservations/clause//elementRef[not(@id = preceding:: clause//elementRef/@id)]/@id";
	private static final String XPATH_ALL_STARTA_ELEMENTREF_ID = "/measure/strata/clause//elementRef[not(@id = preceding:: clause//elementRef/@id)]/@id";
	...
	private static final String XPATH_ALL_MSR_OBS_ATTRIBUTES_UUID = "/measure/measureObservations/clause//attribute[not(@qdmUUID = preceding:: clause//attribute/@qdmUUID)]/@qdmUUID";
	private static final String XPATH_ALL_STARTA_ATTRIBUTES_UUID = "/measure/strata/clause//attribute[not(@qdmUUID = preceding:: clause//attribute/@qdmUUID)]/@qdmUUID";
	...
	/*
	 * @Autowired private HeaderInfoGenerator headerInfoGenerator;
	 * 
	 * @Autowired private ElementLookupGenerator elementLookupGenerator;
	 * 
	 * @Autowired private SuppDataElementsGenerator suppDataElementsGenerator;
	 * 
	 * @Autowired private ClauseBusinessService clauseService;
	 * 
	 * @Autowired private AttachmentGenerator attachmentGenerator;
	 */
	...
	/*
	 * @Autowired private QualityDataSetDAO qdsDAO;
	 * 
	...
	/*
	 * public QualityDataSetDAO getQDSDAO() { return qdsDAO; }
	 */

	@Override
	public ExportResult exportMeasureIntoSimpleXML(String measureId,
	...
	/**
	 * use interimXML to generate and evaluate the emeasure xml persist the
	 * interimXML if a validation failure occurs
	 * 
	 * @param interimXML
	 * @param measureId
	 * @throws Exception
	 */
	/*
	 * private void doXSDValidation(String interimXML, String measureId) throws
	 * Exception{ String emeasureXML = null; emeasureXML =
	 * getEMeasureXML(measureId, interimXML).export; mat.model.clause.Measure
	 * measure = measureDAO.find(measureId); XSDValidationService xvs = new
	 * XSDValidationService(emeasureXML, interimXML, measure,
	 * measureValidationLogDAO); xvs.validateEmeasureXML(); }
	 */
	...
	public ExportResult getEMeasureXML(String measureId,
			MeasureExport measureExport) throws Exception {
		// mat.model.clause.Measure measure = measureDAO.find(measureId);
	...
	/*
	 * public Measure createSimpleXML(String measureId,String xmlString) throws
	 * Exception { Measure m = new Measure(); //headers //New code. //Handle
	 * header information // Get header metadata info from the data. Headers
	 * headers = retrieveMetaDataInfo(measureId); m.setHeaders(headers); String
	 * userOrgOid = ""; User currentUser =
	 * userDAO.find(LoggedInUserUtil.getLoggedInUser()); if(currentUser != null
	 * && currentUser.getOrgOID() != null) { userOrgOid =
	 * currentUser.getOrgOID(); } //element lookup //Fetch Quality Data set from
	 * the database. //QualityDataSetDAO.getQDSElements(measureId, version); //
	 * set qdsel via ElementLookup List<QualityDataSet> qdsList =
	 * qdsDAO.getForMeasure(measureId); Elementlookup datacriteria =
	 * elementLookupGenerator.createElementLookup(qdsList, headers, userOrgOid);
	 ...
	 * 
	 * SupplementalDataElements suppDataElements =
	 * suppDataElementsGenerator.createSuppDataElements(qdsList, headers,
	 * userOrgOid); suppDataElements.getQdsels().addAll(new ArrayList<Qdsel>());
	 * suppDataElements.getIqdsels().addAll(new ArrayList<Iqdsel>());
	 * m.setSupplementalDataElements(suppDataElements);
	 * 
	 * //Load all clauses for this measure. //Filter system clauses.
	 * 
	 * //US 594:- getting the supplimentalQDM Elements added to the measures.
	 * List<QualityDataSet> supplementalQDMS =
	 * suppDataElementsGenerator.getOnlySupplimentalQDMS(qdsList);
	 * handleRules(measureId, m,supplementalQDMS); return m; }
	 */

	/*
	 * private Criterion createCriterionAndAddToMeasure(CriterionToInterim cti,
	 * Measure m, String measureId, Clause clause, boolean doEmptyCheck) throws
	 * Exception { Clause c = populateClauseRules(measureId, clause);
	 * if(doEmptyCheck && isEmpty(c)) return null;
	 * 
	 * Criterion criterion = cti.getCriterion(c);
	 * m.getElementlookup().getMeasurecalcs().addAll(cti.getFunctions());
	 * m.getElementlookup().addPropelsSansDups(cti.getPropels()); List<Qdsel>
	 * qdselsret = cti.getQdsels(); addQdselsToSimpleXML(m,qdselsret);
	 * m.setCriterion(criterion);
	 * 
	 * return criterion; }
	 */
	/*
	 * private boolean isEmpty(Clause c){ if(c.getDecisions().isEmpty()) return
	 * true; if(c.getDecisions().get(0) instanceof Clause) return
	 * isEmpty((Clause) c.getDecisions().get(0)); return false; }
	 */

	// TODO Add Numerator Exclusions
	/*
	 * private void handleRules(String measureId, Measure m,List<QualityDataSet>
	 * supplementalQDMS) throws Exception { Collection<PackageInfo> packages =
	 * attachmentGenerator.buildPackageInfoForMeasure(measureId);
	 * QualityDataSetDAO qualityDataSetDAO = getQDSDAO(); List<QualityDataSet>
	 * qds= qualityDataSetDAO.getForMeasure(measureId); CriterionToInterim cti =
	 * new CriterionToInterim(qds, context);
	 * 
	 * boolean multiplePackages = packages.size() != 1; int counter = 1;
	 * for(PackageInfo packageInfo : packages) { Criterion population =
	 * createCriterionAndAddToMeasure(cti,m, measureId, packageInfo.population,
	 * false);
	 * 
	 * String pTitle = multiplePackages ? "Initial Patient Population " +
	 * counter : "Initial Patient Population"; String dTitle = "Denominator " +
	 * counter; String pClause = packageInfo.population.getName();
	 * 
	 * counter++;
	 * 
	 * 
	 * should check for measure type if a continuous variable measure, then
	 * denominator will be null
	 * 
	 * if(packageInfo.denominator != null){ Criterion denominator =
	 * createCriterionAndAddToMeasure(cti, m, measureId,
	 * packageInfo.denominator, false);
	 * attachmentGenerator.addAttachment((Denominator)denominator, population,
	 * pClause, pTitle); String dClause = packageInfo.denominator.getName();
	 * 
	 * for(Clause numeratorClause : packageInfo.numerators) { Criterion crit =
	 * createCriterionAndAddToMeasure(cti, m, measureId, numeratorClause, true);
	 * boolean addAttachment = isRatioMeasure(m) || (multiplePackages && crit !=
	 * null); if(addAttachment) {
	 * attachmentGenerator.addAttachment((CriterionWithAttachments) crit,
	 * denominator, dClause, dTitle); } } for(Clause exclusionClause :
	 * packageInfo.numExclusions) { Criterion crit =
	 * createCriterionAndAddToMeasure(cti, m, measureId, exclusionClause, true);
	 * if(multiplePackages && crit != null) {
	 * attachmentGenerator.addAttachment((CriterionWithAttachments) crit,
	 * denominator, dClause, dTitle); } }
	 * 
	 * for(Clause exceptionClause : packageInfo.exceptions) {
	 * 
	 * Criterion crit = createCriterionAndAddToMeasure(cti, m, measureId,
	 * exceptionClause, true); if(multiplePackages && crit != null) {
	 * attachmentGenerator.addAttachment((CriterionWithAttachments) crit,
	 * denominator, dClause, dTitle); } } }
	 * 
	 * for(Clause exclusionClause : packageInfo.exclusions) { Criterion crit =
	 * createCriterionAndAddToMeasure(cti, m, measureId, exclusionClause, true);
	 * if(multiplePackages && crit != null) {
	 * attachmentGenerator.addAttachment((CriterionWithAttachments) crit,
	 * population, pClause, pTitle); } } for(Clause measurePopulationClause :
	 * packageInfo.measurePopulation) { Criterion crit =
	 * createCriterionAndAddToMeasure(cti, m, measureId,
	 * measurePopulationClause, true); //an attachment for a measurePopulation
	 * should always be made if(crit != null) {
	 * attachmentGenerator.addAttachment((CriterionWithAttachments) crit,
	 * population, pClause, pTitle); } } for(Clause measureObservationClause :
	 * packageInfo.measureObservation) { Criterion crit =
	 * createCriterionAndAddToMeasure(cti, m, measureId,
	 * measureObservationClause, true); } }
	 * 
	 * US597 - US598 List<Clause> stratifications =
	 * getStrataFromMeasure(measureId); for(Clause stratificationClause :
	 * stratifications) { Criterion crit = createCriterionAndAddToMeasure(cti,
	 * m, measureId, stratificationClause, true); //no attachments are necessary
	 * for stratification system clauses }
	 * 
	...
	/*
	 * private boolean isClauseEmpty(Clause c){ CriterionToInterimUtility ctiu =
	 * new CriterionToInterimUtility(); IQDSTerm term = ctiu.peel(c); return
	 * term.getDecisions().isEmpty(); }
	 */
	 ...
	 */
	/*
	 * private List<Clause> getStrataFromMeasure(String measureId){ Context ctx
	 * = contextDAO.getContext(ConstantMessages.STRAT_CONTEXT_DESC); return
	 * clauseDAO.getAllStratificationClauses(measureId,ctx.getId()); }
	 */

	/*
	 * private void addQdselsToSimpleXML(Measure m, List<Qdsel> qdselsret) { for
	 * (Qdsel q: qdselsret) { if (!m.getElementlookup().getQdsels().contains(q))
	 * { m.getElementlookup().getQdsels().add(q); } } }
	 * 
	 * private Clause populateClauseRules(String measureId, Clause cl) { // TODO
	 * Auto-generated method stub return clauseService.loadClause(measureId,
	 * cl.getId(), context); }
	 */

	/*
	 * private Headers retrieveMetaDataInfo(String measureId) { return
	 * headerInfoGenerator.getAllHeaders(measureId); }
	 */

	/*
	 * private String writeMeasureXML(Measure measure) throws IOException {
	 * 
	 * SimpleXMLWriter sxw = new SimpleXMLWriter(); String x =
	 * sxw.toXML(measure); // System.out.println(x); return x; }
	 */
	...
	public HSSFWorkbook createEMeasureXLS(String measureId,
			List<String> allQDMs, List<String> supplementalQDMS)
			throws Exception {
	...
	private byte[] getHSSFWorkbookBytes(HSSFWorkbook hssfwkbk)
			throws IOException {
		CodeListXLSGenerator clgen = new CodeListXLSGenerator();
	...
	public byte[] getZipBarr(String measureId, MeasureExport me)
			throws Exception {
		byte[] wkbkbarr = null;
		if (me.getCodeList() == null)
	...
	/*
	 * private boolean isRatioMeasure(Measure m){ boolean hasScoringType =
	 * m.getHeaders().getScores() == null ? false :
	 * m.getHeaders().getScores().getScores().isEmpty() ? false : true;
	 * if(hasScoringType){ String scoringType =
	 * m.getHeaders().getScores().getScores().get(0).getTtext(); return
	 * scoringType.equalsIgnoreCase("RATIO"); } return false; }
	 */
	...
	public void createFilesInBulkZip(String measureId, MeasureExport me,

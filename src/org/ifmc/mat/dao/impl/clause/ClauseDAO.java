...
public class ClauseDAO extends GenericDAO<Clause, String> implements org.ifmc.mat.dao.clause.ClauseDAO {
	...
	public Clause loadClauseByName(String measureId, String clauseName, ApplicationContext ctx) {
		...
		org.ifmc.mat.shared.model.Decision sharedDecision = cm.loadClause(clause.getDecision().getId(),ctx);
...
*/	
	...
	public java.util.List<Clause> findSystemClauses(String measureOwnerId, String userRole, ApplicationContext context) {
		...
		if (userRole.equalsIgnoreCase("User")) {//user
			mList = ((org.ifmc.mat.dao.clause.MeasureDAO)context.getBean("measureDAO")).findByOwnerId(measureOwnerId);
		} else {
			mList = ((org.ifmc.mat.dao.clause.MeasureDAO)context.getBean("measureDAO")).find();
		}
	...
	public void clone(String cloneToMeasureId, Clause clause, 
		...
		org.ifmc.mat.dao.clause.MeasureDAO mDAO = (org.ifmc.mat.dao.clause.MeasureDAO)context.getBean("measureDAO");
	...
	private void upgradeClause(ApplicationContext context) {
		...
		List<Measure> mList = ((org.ifmc.mat.dao.clause.MeasureDAO)context.getBean("measureDAO")).find();		
		for (Measure m : mList) {
			String mId = m.getId();
